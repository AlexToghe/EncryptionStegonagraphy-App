...
class StegoApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Salsa20 Encryption Steganography App")
        self.root.geometry("600x500")  # Set window size
        self.root.resizable(True, True)  # Prevent resizing
    ...
    def set_window_icon(self):
        ...
        icon_path = os.path.join(base_path, "lock.ico")
    ...
    def load_image(self):
        ...
        if not file_path:
            messagebox.showinfo("No image selcted", "Please select a PNG image.")
            return
        if not file_path.lower().endswith('.png'):
            messagebox.showerror("Invalid File, Type", "only PNG files are supported. Please select a valid PNG file.")
            return
        ...
        if self.image is None:
            messagebox.showerror("Error Loading Image", "The selected file could not be loaded. Please select a valid PNG image.")
        else:
